...
public class Main_10250 {
	public static void main(String[] args) throws Exception {
		 ...
		 * XX 는 엘리베이터에서부터 세었을 때의 번호를 나타낸다. 즉, 
		 * 그림 1 에서 빗금으로 표시한 방은 305 호가 된다.
		...
		/**
		 * 이해한 내용
		 * 1. 손님이 도착한대로 방배정 함. (손님은 정문에서 가장 가까운 방 선호)
		 * 2. W = 한층에 있는 방 수, H = 층 수, 전체 방수 W x H
		 * 3. 정문과 가까운 방이 1호이고
		 * 3-1. 거리가 가까운 방 우선으로 방이 나가고 거리가 같으면 저층이 우선
		 * 3-2. 101, 201, 301 / 102호 보다는 901호가 나음  
		 * 4. 예제의 첫째줄 입력 -> Test case 수
		 * 4-1. 예제 둘째줄 입력 -> 앞에서 부터 H(층수), W(한층의 방수), N(손님순서)
		 * 
		 * 생각
		 * 1. 방이 빠지는 순서는 101, 201, 301 순으로 빠진다.
		 * 2. 한층의 1호가 다빠져야 다음 1층의 2호가 빠진다.
		 * ?? W값이 의미가 있나? 
		 * */
		printRoomNumber();
	}
	
	/* 입력받은 순선의 손님이 몇번 방에 배정되는지 출력한다. H(층수), W(한층의 방수), N(n번째 손님)*/
	public static void printRoomNumber() throws Exception {
		...
		//테스트 케이스의 수만큼 (층수, 방개수, 손님순서)을 입력받고 방 번호를 출력한다.
		for(int i = 0; i < testCaseCount; i++) {
			String[] inputStringData = new String[3];
			inputStringData = (br.readLine()).split(" ");
			
			//배열의 0번 index: 층수, 1번 index: 방개수, 2번 index: 손님순서
			int[] inputIntegerData = new int[3];
			inputIntegerData[0] = Integer.parseInt(inputStringData[0]);
			inputIntegerData[1] = Integer.parseInt(inputStringData[1]);
			inputIntegerData[2] = Integer.parseInt(inputStringData[2]);
			
			//호 N / H
			int H = inputIntegerData[0];
			int N = inputIntegerData[2];
			
			int XX = (N / H) + 1; //1호부터 시작하므로
			int YY = N % H;
			...
			//YY와 XX를 문자열로 붙인다. 단 XX가 10보다 작으면 앞에 0을 붙여 01, 02와 같이 만든다.
			String roomNumber = String.valueOf(YY) + (XX < 10 ? "0" + XX : String.valueOf(XX));
