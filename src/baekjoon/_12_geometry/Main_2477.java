...
public class Main_2477 {
	public static void main(String[] args) throws Exception {
		 ...
		 * 문제
			...
			이 그림의 왼쪽위 꼭짓점에서 출발하여, 반시계방향으로 남쪽으로 30m, 동쪽으로 60m, 남쪽으로 20m, 
...
*/		
		// 4(북)50  3(남)30 3(남)20
		// 2(서)160 1(동)60 1(동)100
		
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		int fruitCntInArea = Integer.parseInt(br.readLine()); //1m^2당 참외 개수
		
		//1(동) 2(서) 3(남) 4(북)
		
		String direction = "";
		int[] dirLength = new int[12];
		
		for(int i = 0; i < 6; i++) {
			String[] str = br.readLine().split(" ");
			direction += str[0];
			dirLength[i] = Integer.parseInt(str[1]);
		}
		...
		int k = 0;
		for(int i = 6; i < dirLength.length; i++) {
			dirLength[i] = dirLength[k];
			k++;
		}
		
		//System.out.println(Arrays.toString(dirLength));
		
		// 1. 가장 긴 변 2개 찾기
		int maxLength1 = 0;
		for(int i = 0; i < direction.length(); i++) {
			if(direction.charAt(i) == '1' || direction.charAt(i) == '2') {
				maxLength1 += dirLength[i];
			}
		}
		
		int maxLength2 = 0;
		for(int i = 0; i < direction.length(); i++) {
			if(direction.charAt(i) == '3' || direction.charAt(i) == '4') {
				maxLength2 += dirLength[i];
			}
		}
		
		maxLength1 = maxLength1 / 4;
		maxLength2 = maxLength2 / 4;
		
		
		//남동남(313) 서남서(232) 북서북(424) 동북동(141) 찾기
		int idx = 0;
		if(direction.contains("313")) {
			idx = direction.indexOf("313");
		}else if(direction.contains("232")) {
			idx = direction.indexOf("232");
		}else if(direction.contains("424")) {
			idx = direction.indexOf("424");
		}else {
			idx = direction.indexOf("141");
		}
		
		int result = 0;
		
		int totalArea = maxLength1 * maxLength2; //참외밭 전체 직사각형 넓이
		int minusArea = dirLength[idx+1] * dirLength[idx+2]; //참외밭이 아닌 영역  
		
		result = (totalArea - minusArea) * fruitCntInArea;
