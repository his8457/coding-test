...
public class Main_3003 {
	...
	private static void solve() throws Exception {
		...
		int[] inputUnitArr = new int[6]; //킹, 퀸, 룩, 비숍, 나이트, 폰
		
		int i = 0;
		for(String s : inputStr.split(" ")) {
			if(i == 0 || i == 1) {//킹
				inputUnitArr[i] = 1 - Integer.parseInt(s);
			}else if(i == 2 || i == 3 || i == 4) {
				inputUnitArr[i] = 2 - Integer.parseInt(s);
			}else {
				inputUnitArr[i] = 8 - Integer.parseInt(s);
			}
			i++;
		}
		
		StringBuilder sb = new StringBuilder();
		for(i = 0; i < inputUnitArr.length; i++) {
			sb.append(inputUnitArr[i] + " ");
		}
